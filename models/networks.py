...
from speechbrain.lobes.models.ECAPA_TDNN import AttentiveStatisticsPooling, BatchNorm1d, Conv1d
...
class FiLM(nn.Module):
    def __init__(self, input_size, feature_size):
        ...
        self.fc_gamma = nn.Linear(input_size, feature_size)
        self.fc_beta = nn.Linear(input_size, feature_size)
        self.norm = nn.InstanceNorm2d(256, affine=True)
        self.prelu = nn.PReLU()

    def forward(self, embedding, feature):
        """
        embedding: input to the FiLM generator (could be the same input to the network or some other input)
        feature: intermediate feature maps from the target network
        """
        gamma = self.fc_gamma(embedding)
        beta = self.fc_beta(embedding)
        # print(f'embedding: {embedding.shape}')
        # print(f'gamma: {gamma.shape}\tbeta: {beta.shape}')

        FiLMed_feature = self.prelu(gamma * self.norm(feature) + beta)
        # print(f'FiLMed_feature: {FiLMed_feature.shape}')

        return FiLMed_feature
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        if self.opt.inject_type == 'FiLM':
            self.film_layers = nn.ModuleList([FiLM(input_size=256, feature_size=ngf) for _ in range(10)])

    def forward(self, input, layers=[], encode_only=False, channel_emb=None, inject_layers=[]):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in inject_layers and channel_emb is not None:
                    if self.opt.inject_type == 'FiLM':
                        feat = self.film_layers[film_idx](channel_emb, feat)
                        film_idx += 1
                    else:
                        pass
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in inject_layers and channel_emb is not None:
                    # print(f'feat_before: {feat.shape}\tchannel_emb: {channel_emb.shape}')
                    if self.opt.inject_type == 'FiLM':
                        feat = self.film_layers[film_idx](channel_emb, feat)
                        film_idx += 1
                    else:
                        pass
                    # print(f'feat_after: {feat.shape}')
