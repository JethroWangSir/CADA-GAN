...
def split_and_save(spec, spec_w, pow=1.0, state = "Train", channels = 1):
    """
...
def processInput(filepath, spec_w, power, state, channels):
    mag_spec, phase, sr = functions.extract(filepath, sr=16000, energy=1.0, state = state)
    components = split_and_save(mag_spec, spec_w, pow=power, state = state, channels = channels)
...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.state == "Test":
            self.dir_A = os.path.join(f"/share/nas169/jethrowang/CADA-GAN/tat_asr_aligned/UNA-GAN/source/{opt.source_folder}")
            self.dir_B = os.path.join("/share/nas169/jethrowang/CADA-GAN/tat_asr_aligned/UNA-GAN/trainB")           
        ...
        if("passcodec" in opt.preprocess):
            ...
            for path in self.A_paths:
                subprocess.call(['ffmpeg', '-hide_banner', '-loglevel', 'error', '-i', path, '-ar', '16k', '-y', path[:-4] + '_16k.wav'])
                subprocess.call(['ffmpeg', '-hide_banner', '-loglevel', 'error', '-i', path[:-4] + '_16k.wav', '-acodec', 'g726', '-b:a', '16k', path[:-4] + '_fmt.wav'])
                subprocess.call(['ffmpeg', '-hide_banner', '-loglevel', 'error', '-i', path[:-4] + '_fmt.wav', '-ar', '16k', '-y', path])
                if(os.name == 'nt'):  # Windows
                    os.system('del ' + path[:-4] + '_fmt.wav')
                    os.system('del ' + path[:-4] + '_16k.wav')
                else:  # Linux/MacOS/BSD
                    os.system('rm ' + path[:-4] + '_fmt.wav')
                    os.system('rm ' + path[:-4] + '_16k.wav')
        ...
        self.clean_specs = Parallel(n_jobs=self.num_cores, prefer="threads")(delayed(processInput)(i, opt.fix_w, self.spec_power, self.state, self.channels) for i in self.A_paths)
        ...
        self.noisy_specs = Parallel(n_jobs=self.num_cores, prefer="threads")(delayed(processInput)(i, opt.fix_w, self.spec_power, self.state, self.channels) for i in self.B_paths)
        self.no_comps_noisy = Parallel(n_jobs=self.num_cores, prefer="threads")(delayed(countComps)(i) for i in self.noisy_specs)
        self.noisy_spec_paths = []
        self.noisy_comp_dict = OrderedDict()
        for nameB, countB in zip(self.B_paths, self.no_comps_noisy):
        ...
        if self.state == "Test" and self.opt.random_inference == 1:
            zipped_paths = list(zip(self.B_paths))
            random.shuffle(zipped_paths)
            self.shuffled_B_paths = zip(*zipped_paths)
            self.shuffled_B_paths = list(self.shuffled_B_paths)
    ...
    def __getitem__(self, index):
        ...
        if(self.state == "Train"):
            ...
            if self.opt.random_inference == 1:
                index_B = index % len(self.shuffled_B_paths)
                B_path = self.shuffled_B_paths[index_B]
                B_img = self.noisy_specs[self.B_paths.index(B_path)]
                B = transform(B_img)
            else:
                index_B = index % self.noisy_specs_len
                B_path = self.noisy_spec_paths[index_B]
